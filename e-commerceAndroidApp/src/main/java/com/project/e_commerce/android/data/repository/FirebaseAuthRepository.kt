...
class FirebaseAuthRepository(
    ...
    override suspend fun signInWithFacebook(accessToken: String): Result<FirebaseUser> = runCatching {
        Log.d("FirebaseAuth", "Starting Facebook Sign-In with accessToken")
        ...
        Log.d("FirebaseAuth", "Facebook Sign-In successful for user: ${user.email}")
        ...
        if (!userDoc.exists()) {
            ...
            try {
                ...
                Log.d("FirebaseAuth", "User profile created successfully in Firestore")
            } catch (e: Exception) {
                Log.e("FirebaseAuth", "Failed to create user profile in Firestore", e)
                // Don't fail the sign-in process if Firestore write fails
            }
        }

        user
    }.recoverCatching { exception ->
        Log.e("FirebaseAuth", "Facebook Sign-In failed", exception)
        when (exception) {
            is FirebaseAuthException -> {
                when (exception.errorCode) {
                    "ERROR_ACCOUNT_EXISTS_WITH_DIFFERENT_CREDENTIAL" ->
                    ...
                    else -> throw Exception("فشل تسجيل الدخول بفيسبوك: ${exception.message}")
                }
            }

            else -> throw Exception("حدث خطأ أثناء تسجيل الدخول بفيسبوك")
        }
    }.mapError()
