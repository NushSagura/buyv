    ...
    override fun onClickLoginByGoogle(navController: NavController) {
        ...
        viewModelScope.launch {
            try {
                ...
                // This will be handled by the UI layer with activity result launcher
                setLoadingState(false)
            } catch (e: Exception) {
                setLoadingState(false)
                setErrorState(true, "خطأ في تسجيل الدخول بـ Google: ${e.message}")
            }
        }
    }
    ...
    override fun onClickLoginByFacebook(navController: NavController) {
        ...
        viewModelScope.launch {
            try {
                ...
                setLoadingState(false)
                setErrorState(true, "خطأ في تسجيل الدخول بـ Facebook: ${e.message}")
            }
        }
    }
